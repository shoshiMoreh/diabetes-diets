...
from pywin32_bootstrap import path
...
daysMeal = ['breakfast', 'lunch', 'dinner', 'snack1', 'snack2']
...
# def randomDatasetMeal(datas):
#     per10 = len(datas) / 10
#     per15 = len(datas) / 15
#     per35 = len(datas) / 35
#     per30 = len(datas) / 30
#     n = [0, per15, per15+per35, per15+per35+per30, per15+per35+per30+per10, per15+per35+per30+per10+per10-1]
#     mixData = datas.sample(frac=1).reset_index().drop('index', axis=1)
#     mealDay = []
#     for s in range(len(n) - 1):
#         mealDay.append(mixData.loc[n[s]:n[s + 1]])
#     return dict(zip(daysMeal, mealDay))
...
# t = totalModel(70, numCalories(80,170, 35, 1))
# print(type(t))
# for day in daysWeek:
#     print(day)
#     print(t[day])
